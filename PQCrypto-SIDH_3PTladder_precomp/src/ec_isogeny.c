...
void xADD1(const felm_t xpre, point_proj_t Q, const f2elm_t XPQ, const f2elm_t ZPQ)
{
    felm_t t0 = { 0 }, t1 = { 0 };
    fpcopy(xpre, t0);
    fpsub(Q->X[0], Q->Z[0], t1);
    fpmul_mont(t1, t0, t0);
    fpadd(Q->X[0], Q->Z[0], t1);
    fpsub(t0, t1, Q->Z[0]);
    fpadd(t0, t1, Q->X[0]);
    fpsqr_mont(Q->Z[0], Q->Z[0]);
    fpsqr_mont(Q->X[0], Q->X[0]);
    fpmul_mont(Q->Z[0], XPQ[0], Q->Z[0]);
    fpmul_mont(Q->X[0], ZPQ[0], Q->X[0]);
}


void xADD2(const felm_t xQ2, point_proj_t Q, const f2elm_t XPQ, const f2elm_t ZPQ)
{
    f2elm_t t0 = { 0 }, t1 = { 0 }, t2 = { 0 };
    felm_t One;
    fpcopy((digit_t*)&Montgomery_one, One);
    mp_addfast(xQ2, One, t0[0]);
    mp_sub_p2(xQ2, One, t1[0]);
    mp2_sub_p2(Q->X, Q->Z, t2);
    fpmul_mont(t2[1], t0[0], t0[1]);
    fpmul_mont(t2[0], t0[0], t0[0]);
    mp2_add(Q->X, Q->Z, t2);
    fpmul_mont(t2[1], t1[0], t1[1]);
    fpmul_mont(t2[0], t1[0], t1[0]);
    mp2_sub_p2(t0, t1, Q->Z);
    mp2_add(t0, t1, Q->X);
    fp2sqr_mont(Q->Z, Q->Z);
    fp2sqr_mont(Q->X, Q->X);
    fp2mul_mont(Q->Z, XPQ, Q->Z);
    fp2mul_mont(Q->X, ZPQ, Q->X);
}
...
static void LADDER3PT_for_Alice(const felm_t xQ2, const digit_t* m, point_proj_t R, point_proj_t R2)
{
    felm_t* pre;
    digit_t mask;
    int i, nbits, bit, swap, prevbit = 0;
    ...
    // Initializing points
    pre = (felm_t*)pre_for_Alice;
    fpcopy(*pre, (digit_t*)(R->X)[0]);
    fpcopy((digit_t*)&Montgomery_one, (digit_t*)(R->Z)[0]);
    pre = pre + 1;
    fpcopy(*pre, (R2->X)[0]);
    fpcopy((digit_t*)&Montgomery_one, (digit_t*)(R2->Z)[0]);
...
static void LADDER3PT_for_Bob(const felm_t xP, const digit_t* m, point_proj_t R, point_proj_t R2)
{
    digit_t mask;
    felm_t* pre;
    int i, nbits, bit, swap, prevbit = 0;
    ...
    pre = (felm_t*)pre_for_Bob + 2;
    // Initializing points
    fpcopy((digit_t*)&Montgomery_one, (digit_t*)(R->X)[0]);
    fpcopy((digit_t*)&Montgomery_one, (digit_t*)(R->Z)[0]);
    fpcopy(*pre, (R2->X)[0]);
    fpcopy((digit_t*)&Montgomery_one, (digit_t*)(R2->Z)[0]);
...
static void LADDER3PT2(const f2elm_t xP, const f2elm_t xQ, const f2elm_t xPQ, const digit_t* m, const unsigned int AliceOrBob, point_proj_t R, const f2elm_t A)
{
    point_proj_t R0 = { 0 }, R2 = { 0 };
    f2elm_t A24 = { 0 };
    digit_t mask;
...
static void RecoverY3PT(point_proj_t R0, point_proj_t R1, point_proj_t R2, point_full_proj_t tR)
{
    felm_t t0, t1, x, y;
    fpcopy(tR->X[0], x);
    fpcopy(tR->Y[0], y);
    ...
    fpmul_mont(t0, R0->Z[0], t0);
    fpmul_mont(t0, R1->Z[0], t0);
    fpmul_mont(t0, R2->Z[0], t0);
    fpmul_mont(t0, R0->X[0], tR->X[0]);
    fpmul_mont(t0, R0->Z[0], tR->Z[0]);
    fpmul_mont(R1->X[0], R2->Z[0], t0);
    fpmul_mont(R1->Z[0], R2->X[0], t1);
    fpsub(t0, t1, t0);
    fpmul_mont(x, R0->Z[0], t1);
    fpsub(R0->X[0], t1, t1);
    fpsqr_mont(t1, t1);
    fpmul_mont(t0, t1, tR->Y[0]);
}


static void plus_for_Alice3(point_full_proj_t tR, point_full_proj_t tR0, point_proj_t R)
{
    felm_t tx = { 0 };
...
static void plus_for_Alice1(point_full_proj_t tR, point_full_proj_t tR0, point_full_proj_t R)
{
    felm_t tx = { 0 }, tx0 = { 0 }, y3 = { 0 }, u = { 0 }, u2 = { 0 }, u3 = { 0 }, v = { 0 }, v2 = { 0 }, v3 = { 0 }, a = { 0 }, r = { 0 };
    fpadd((tR->X)[0], (tR->Z)[0], tx);
    fpadd(tx, (tR->Z)[0], tx);
    fpadd((tR0->X)[0], (tR0->Z)[0], tx0);
    fpadd(tx0, (tR0->Z)[0], tx0);
    fpmul_mont((tR0->Y)[0], (tR->Z)[0], u);
    fpsub(u, (tR->Y)[0], u);
    fpsqr_mont(u, u2);
    fpmul_mont(tx0, (tR->Z)[0], v);
    fpsub(v, tx, v);
    fpsqr_mont(v, v2);
    fpmul_mont(v, v2, v3);
    fpmul_mont(v2, tx, r);
    fpmul_mont(u2, (tR->Z)[0], a);
    fpsub(a, v3, a);
    fpsub(a, r, a);
    fpsub(a, r, a);
    fpmul_mont(v, a, (R->X)[0]);
    fpmul_mont(v3, (tR->Y)[0], y3);
    fpsub(r, a, (R->Y)[0]);
    fpmul_mont(u, (R->Y)[0], (R->Y)[0]);
    fpsub((R->Y)[0], y3, (R->Y)[0]);
    fpmul_mont(v3, (tR->Z)[0], (R->Z)[0]);
    fpsub((R->X)[0], (R->Z)[0], (R->X)[0]);
    fpsub((R->X)[0], (R->Z)[0], (R->X)[0]);
}


static void plus_for_Alice2(const digit_t* m, point_full_proj_t tR)
{
    int bit = 0;
    point_full_proj_t tR1 = { 0 }, tR2 = { 0 };
    fpcopy((digit_t*)&Montgomery_one, (tR1->X)[0]);
    fpcopy((digit_t*)&YQA4, (tR1->Y)[0]);
    fpcopy((digit_t*)&Montgomery_one, (tR1->Z)[0]);
    plus_for_Alice1(tR, tR1, tR2);
    bit = (m[(OALICE_BITS - 2) >> LOG2RADIX] >> ((OALICE_BITS - 2) & (RADIX - 1))) & 1;
    swap_points2(tR, tR2, -bit);
    fpmul_mont((tR->X)[0], (tR->Z)[0], (tR2->X)[0]);
    fpmul_mont((tR->Y)[0], (tR->Z)[0], (tR2->Y)[0]);
    fpneg((tR2->Y)[0]);
    fpmul_mont((tR->X)[0], (tR->X)[0], (tR2->Z)[0]);
    bit = (m[(OALICE_BITS - 1) >> LOG2RADIX] >> ((OALICE_BITS - 1) & (RADIX - 1))) & 1;
    swap_points2(tR, tR2, -bit);
}
